...
pub fn run_tick(
    ...
    let mut current_work_queue = std::mem::take(&mut state.work_queue);
    let mut current_mailboxes = std::mem::take(&mut state.mailboxes);

    let scheduled_nodes: Vec<_> = current_work_queue
        .drain(..)
        .enumerate()
        .map(|(queue_index, node_id)| {
            ...
            let local_inbox = current_mailboxes.remove(&node_id).unwrap_or_default();
            Ok((queue_index, node, local_inbox))
        })
        .collect::<Result<Vec<_>, _>>()?;

    let mut node_results = scheduled_nodes
        .into_par_iter()
...
fn eval_stmt(stmt: &LStmt, ctx: &mut EvalCtx<'_>) -> Result<(), LocatedRuntimeError> {
    match &stmt.value {
        ...
        Stmt::If { cond, yes, no } => {
            ...
            if eval_expr(cond, ctx)?.truthy().map_err(|error| (cond.loc.clone(), error))? {
                eval_stmt(yes, ctx)?;
        ...
        Stmt::Send { value, to } => {
            ...
            let target = match target {
                ...
                _ => return Err((to.loc.clone(), RuntimeError::Type("send target must be a string"))),
            };
        ...
        Stmt::SetColor { value } => {
            ...
            let color = match color {
                Value::Str(value) => value,
                _ => return Err((value.loc.clone(), RuntimeError::Type("set() expects a string"))),
            };
            *ctx.color = color;
        }
...
fn eval_expr(expr: &LExpr, ctx: &mut EvalCtx<'_>) -> Result<Value, LocatedRuntimeError> {
    match &expr.value {
        ...
        Expr::Var(name) => ctx
            ...
            .get(name)
            .cloned()
            .ok_or_else(|| (expr.loc.clone(), RuntimeError::MissingVariable(name.clone()))),
        Expr::Len(inner) => {
            let value = eval_expr(inner, ctx)?;
            match value {
                Value::Str(value) => Ok(Value::Int(value.chars().count() as i64)),
                _ => Err((inner.loc.clone(), RuntimeError::Type("len() expects a string"))),
            }
        }
        Expr::Receive { from } => {
        ...
        Expr::Index { base, index } => {
            ...
            let text = base_value.as_str().map_err(|error| (base.loc.clone(), error))?;
            let index_value = index_value.as_int().map_err(|error| (index.loc.clone(), error))?;
        ...
        Expr::Slice { base, start, end } => {
            ...
            let start = match start {
                Some(start) => eval_expr(start, ctx)?.as_int().map_err(|error| (start.loc.clone(), error))?,
                None => 0,
            };
            let end = match end {
                Some(end) => eval_expr(end, ctx)?.as_int().map_err(|error| (end.loc.clone(), error))?,
                None => chars.len() as i64,
            };
            let start: usize = start.try_into().map_err(|_| (expr.loc.clone(), RuntimeError::OutOfBounds))?;
            let end: usize = end.try_into().map_err(|_| (expr.loc.clone(), RuntimeError::OutOfBounds))?;
        ...
        Expr::UnaryNot(inner) => {
            let value = eval_expr(inner, ctx)?;
            Ok(Value::Bool(!value.truthy().map_err(|error| (inner.loc.clone(), error))?))
        }
        Expr::Binary { op, left, right } => eval_binary(*op, left, right, ctx),
    }
}

fn eval_binary(op: BinaryOp, left: &LExpr, right: &LExpr, ctx: &mut EvalCtx<'_>) -> Result<Value, LocatedRuntimeError> {
    match op {
        ...
        BinaryOp::Add => {
            ...
            match (left_value, right_value) {
                ...
                _ => Err((left.loc.clone(), RuntimeError::Type("+ expects matching ints or strings"))),
            }
        }
...
impl<'a> Parser<'a> {
    ...
    fn parse_program(mut self) -> Result<LStmt, ParseError> {
        let start = self.tokens.first().map(|token| token.loc.clone()).unwrap_or_else(|| loc_for(self.source, 0, 0));
        let mut stmts = Vec::new();
        while !self.is_done() {
            stmts.push(self.parse_stmt()?);
        }
        let end = stmts.last().map(|stmt| stmt.loc.clone()).unwrap_or(start.clone());
        Ok(Loc {
    ...
    fn parse_if(&mut self) -> Result<LStmt, ParseError> {
        ...
        let end_loc = no.as_ref().map(|stmt| stmt.loc.clone()).unwrap_or_else(|| yes.loc.clone());
    ...
    fn parse_postfix(&mut self) -> Result<LExpr, ParseError> {
        ...
        while self.match_symbol("[") {
            ...
            if self.match_symbol(":") {
                let end = if self.peek_symbol("]") { None } else { Some(Box::new(self.parse_expr()?)) };
        ...
        match token.kind {
            ...
            TokenKind::Ident(name) => match name.as_str() {
                ...
                "receive" | "recive" => {
                    ...
                    Ok(merge_loc(&token.loc, &end).with(Expr::Receive { from: Box::new(from) }))
                }
...
fn loc_for(source: &str, start: usize, end: usize) -> Loc {
    Loc {
        range: start..end,
        line_range: line_for(source, start)..line_for(source, end.saturating_sub(1)).saturating_add(1),
    }
}
...
fn line_for(source: &str, offset: usize) -> usize {
    ...
    source[..=capped].bytes().filter(|byte| *byte == b'\n').count() + 1
}
...
"#
        ...
        assert_eq!(tick.log.final_colors.get("left"), Some(&"green".to_string()));
        assert_eq!(tick.log.final_colors.get("right"), Some(&"black".to_string()));
        assert_eq!(tick.log.executed[0].loc.line_range, 2..3);
        assert_eq!(tick.log.messages[0].to, "left");
        assert_eq!(state.work_queue.iter().collect::<Vec<_>>(), vec![&"left".to_string()]);
        assert_eq!(mailbox_values(&state, "left", "right"), vec![Value::Str("xxgreen".into())]);
    }
...
"#
        ...
        assert_eq!(first_tick.log.final_colors.get("center"), Some(&"gray".to_string()));
        assert_eq!(state.work_queue.iter().collect::<Vec<_>>(), vec![&"left".to_string()]);
        assert_eq!(mailbox_values(&state, "left", "center"), vec![Value::Str("pong".into())]);
        ...
        assert_eq!(second_tick.log.final_colors.get("center"), Some(&"blue".to_string()));
    }
}
